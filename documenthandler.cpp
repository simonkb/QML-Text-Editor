...
void DocumentHandler::mergeFormatOnWordOrSelection(const QTextCharFormat &format)
{
    ...
    QTextCharFormat newFormat = existingFormat;

    if (format.fontWeight() != existingFormat.fontWeight())
        newFormat.setFontWeight(format.fontWeight());

    if (format.fontItalic() != existingFormat.fontItalic())
        newFormat.setFontItalic(format.fontItalic());

    if (format.fontUnderline() != existingFormat.fontUnderline())
        newFormat.setFontUnderline(format.fontUnderline());

    if (format.foreground() != existingFormat.foreground())
        newFormat.setForeground(format.foreground());

    if (format.fontPointSize() != existingFormat.fontPointSize())
        newFormat.setFontPointSize(format.fontPointSize());
    // if (format.fontFamily() != existingFormat.fontFamily())
    //     newFormat.setFontFamily(format.fontFamily());
